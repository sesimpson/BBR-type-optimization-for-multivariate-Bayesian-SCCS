		...
		proc sql;
			...
			FROM &outlib..SCCS_PERIODS P
				...
				INNER JOIN &outlib..&drugs D
					...
					WHERE DE.DRUG_ERA_START_DATE <= P.PERIOD_END_DATE
						AND DE.DRUG_ERA_END_DATE > P.PERIOD_START_DATE
	...
	%macro create_sccs_cases_per_period(inlib, outlib, conditions, occur_type);
		...
		proc sql;
			...
			SELECT P.PERSON_ID,
				...
				COUNT(CE.CONDITION_ERA_ID) AS PERIOD_CASE_COUNT
					FROM &outlib..SCCS_PERIODS P
				LEFT JOIN
				&inlib..CONDITION_ERA CE
					ON P.PERSON_ID = CE.PERSON_ID
				INNER JOIN &outlib..&conditions C	
					ON C.condition_concept_id = CE.condition_concept_id
			WHERE CE.CONDITION_ERA_START_DATE > P.PERIOD_START_DATE
				AND CE.CONDITION_ERA_END_DATE <= P.PERIOD_END_DATE	
				AND CE.condition_occurrence_type EQ "&occur_type"
			GROUP BY P.PERSON_ID,
				P.PERIOD_START_DATE,
				P.PERIOD_END_DATE,
				CE.CONDITION_CONCEPT_ID
			ORDER BY CONDITION_CONCEPT_ID, PERSON_ID, PERIOD_START_DATE, PERIOD_END_DATE;
		quit;
		run;
...
*/
	...
	%onetable_SCCS_multivar(/data1/SAS_CDM/GE, GE, /omop1/dev/ssimpson/GE_MLmulti_10x10, GEmult10,
		REAL_10_COI, 65, REAL_10_DOI, 7, nodeletetables, nosystemcall);
	run;
